...
import ProjectTransliteration from '@/components/ProjectTransliteration';
import ProjectImgcap from '@/components/ProjectImgcap';
import ProjectPca from '@/components/ProjectPca';
...
<template>
	<div class="block sm:flex gap-0 sm:gap-10 mt-14">
		...
		<div class="w-full sm:w-1/3 text-left sticky top-14">
		...
		<div class="w-full sm:w-2/3 text-left mt-10 sm:mt-0">
			...
			<div v-for="projectDetail in projectInfo.projectDetails" :key="projectDetail.id"
				class="font-general-regular mb-5 text-lg text-ternary-dark dark:text-ternary-light">
				{{ projectDetail.details }}

				<div v-if="projectDetail.img">
					<b>
						Architecture:
					</b>
				</div>
				<img v-if="projectDetail.img" :src="projectDetail.img"
					class="rounded-xl cursor-pointer shadow-lg sm:shadow-none border-none"
					alt="{{ projectDetail.title }}">
				<div v-if="projectDetail.architecture">
					<b>Architecture:</b>
					<ProjectTransliteration />
				</div>
				<div v-if="projectDetail.archi">
					<b>Architecture:</b>
					<ProjectImgcap />
				</div>

				<div v-if="projectDetail.flow">
					<b>Architecture:</b>
					<ProjectPca />
				</div>
